<template>
  <div class="rss-rule fn-page">
    ...
    <a-table
      ...
      :locale="tableLocale"
      size="small"
      :data-source="filterAdminList(rssRuleList, ['alias', 'id'])"
        ...
        <template v-if="column.dataIndex === 'downloader'">
          <a-select size="small" :allowClear="true" v-model:value="record.client" style="width: 100%;" @change="modifyRssRuleDownloader(record)">
            <template v-for="downloader of downloaders" :key="downloader.id">
              <a-select-option :value="downloader.id">
...
export default {
  ...
  methods: {
    ...
    openCreate () {
      ...
      if (typeof this.clearRssRule === 'function') this.clearRssRule();
