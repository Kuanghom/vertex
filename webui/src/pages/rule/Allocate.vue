<template>
  <div style="font-size: 24px; font-weight: bold;">分配规则</div>
  <a-divider></a-divider>
  <div class="allocate-rule">
    <a-table
    ...
    >
      <template #title>
        <span style="font-size: 16px; font-weight: bold;">分配方案列表</span>
      </template>
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.title === '操作'">
          <span>
            <a @click="openApplyRss(record)">应用到 RSS</a>
            <template v-if="!record.builtin">
              ...
              <a-popover title="删除?" trigger="click" :overlayStyle="{ width: '84px', overflow: 'hidden' }">
                <template #content>
                  <a-button type="primary" danger @click="deleteAllocateRule(record)" size="small">删除</a-button>
                </template>
                <a style="color: red">删除</a>
              </a-popover>
            </template>
            <a-tag v-else style="margin-left: 8px;">内置</a-tag>
          </span>
        </template>
      </template>
    </a-table>
    <a-divider></a-divider>
    <div style="font-size: 16px; font-weight: bold; padding-left: 8px;">新增 | 编辑自定义分配规则</div>
    <div style="text-align: left; ">
      <a-form
        ...
        <a-form-item
          ...
          <a-button
            v-if="allocateRule.id"
            style="margin-left: 12px; margin-top: 24px; margin-bottom: 48px;"
            @click="openApplyRss(allocateRule)">
            应用到 RSS
          </a-button>
          <a-button style="margin-left: 12px; margin-top: 24px; margin-bottom: 48px;" @click="clearAllocateRule()">清空</a-button>
        </a-form-item>
      </a-form>
      <a-divider></a-divider>
      ...
      <a-form
        ...
        <a-form-item :wrapperCol="isMobile() ? { span:24 } : { span: 21, offset: 3 }">
          ...
          <a-button style="margin-left: 12px;" @click="clearDebug()">清空输出</a-button>
        </a-form-item>
        <a-form-item v-if="debugResult.rounds" label="分配结果">
          <a-table
            size="small"
            :pagination="false"
            :columns="debugRoundColumns"
