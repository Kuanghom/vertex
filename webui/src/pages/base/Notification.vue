    ...
    >
      <a-form
        ...
        <a-form-item
          label="类型"
          name="type"
          extra="钉钉、IYUU、MoviePilot、企业微信群机器人已由后端直接发送，无需再走通用 Webhook"
          :rules="[{ required: true, message: '${label}不可为空! ' }]">
          <a-select size="small" v-model:value="notification.type"  >
            ...
            <a-select-option value="moviepilot">MoviePilot</a-select-option>
        ...
        <a-form-item
          v-if="notification.type === 'moviepilot'"
          label="地址"
          name="moviepilotUrl"
          extra="MoviePilot 访问地址。走「外部消息转发 / MsgNotify」插件发出，同一企微不要两边都开"
          :rules="[{ required: true, message: '${label}不可为空! ' }]">
          <a-input size="small" v-model:value="notification.moviepilotUrl" placeholder="http://192.168.1.10:3000"/>
        </a-form-item>
        <a-form-item
          v-if="notification.type === 'moviepilot'"
          label="API 令牌"
          name="moviepilotToken"
          extra="设定 → 系统 → API令牌。若已填写完整插件地址，令牌仍会作为 apikey 附加"
          :rules="[{ required: true, message: '${label}不可为空! ' }]">
          <a-input size="small" v-model:value="notification.moviepilotToken"/>
        </a-form-item>
        <a-form-item
