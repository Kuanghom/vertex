...
export default {
  ...
  computed: {
    ...
    refGroups () {
      ...
      if ((refs.douban || []).length) groups.push({ key: 'douban', title: '豆瓣订阅', path: '/task/subscribe', items: refs.douban });
      if ((refs.watch || []).length) groups.push({ key: 'watch', title: '监控分类', path: '/task/watchCategory', items: refs.watch });
