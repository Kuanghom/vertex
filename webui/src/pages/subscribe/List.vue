<template>
  ...
  <fn-filter :active="type !== 'all' || status !== 'all'" title="筛选">
    ...
    <div class="fn-filter-item">
      <span>状态</span>
      <a-select v-model:value="status" @change="doFilte">
        <a-select-option value="all">全部</a-select-option>
  ...
  <div class="subscribe-list">
    <div
      ...
      :style="`display: inline-block; margin: 12px; text-align: center; width: ${isNarrow ? '160px' : '200px'}; vertical-align: top;`">
      <div :class="isNarrow ? 'item-class-mobile' : 'item-class-pc'">
      ...
      <div style="margin: 6px auto; max-width: 100%;" v-if="isNarrow">
        <div style="height: 48px;">
          <fa v-if="item.downloaded" style="font-size: 16px; color: green;" :icon="['fas', 'check']"></fa>
          {{item.name.split('/')[0]}}
        </div>
      </div>
      <div style="margin: 6px auto;" v-if="!isNarrow">
        <div style="height: 48px;">
          <fa v-if="item.downloaded" style="font-size: 16px; color: green;" :icon="['fas', 'check']"></fa>
