    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.title === '操作'">
          <fn-ops>
            <a-button type="link" @click="refreshSubscribe(record)">刷新</a-button>
            <a-button type="link" @click="modifyClick(record)">编辑</a-button>
            <a-button type="link" @click="cloneClick(record)">克隆</a-button>
            <a-popconfirm title="确认删除这条数据？" ok-text="删除" cancel-text="取消" @confirm="deleteSubscribe(record)">
...
export default {
  ...
  methods: {
    ...
    openCreate () {
      this._formEditing = false;
      if (typeof this.clearSubscribe === 'function') this.clearSubscribe();
