...
class SiteMod {
  ...
  async listRecord () {
    ...
    const siteList = util.listSite().filter(i => !!global.runningSite[i.name])
      .map(item => { return { ...item, used: rssList.some(i => (i.sites || []).indexOf(item.name) !== -1) }; });
  ...
  async list () {
    ...
    const siteList = util.listSite()
      .map(item => { return { ...item, used: rssList.some(i => (i.sites || []).indexOf(item.name) !== -1), index: global.SITE.siteUrlMap[item.name] }; });
    for (let site of siteList) {
      site = Object.assign(site, global.runningSite[site.name]?.info || {});
    }
    const increase = {
